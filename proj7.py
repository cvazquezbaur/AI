...
class EightPuzzle:
    ...
    #follows the algorithm layed out in pseudocode on F-13
    def bestFirst(self, start, state_idx):
         open = [start]
         closed = []
         while(open.len() > 0):
             ...
             if(currentState == goal):
                 return currentState
             currentState.generate_states(state_idx)             
             for state in self.state_list:
                hasChanged = false
                for itemB in closed:
                    if (state == itemB):
                        if(state.state_idx < itemB.state_idx):
                            closed.remove(itemB)
                            open.append(state)
                            hasChanged = true
                    for itemA in open:
                        if (state == itemA):
                            if(state.state_idx < itemA.state_idx):
                                itemA.state_idx = iteam.state_idx
                                hasChanged = true                       
                        
                if (hasChanged == false):
                    heuristic = state.state_idx + self.state_lst.length()
                    open.sort()
         return false
