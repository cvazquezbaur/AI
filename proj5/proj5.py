...
class EightPuzzle:
    ...
    def depth_first_search(self, start, goal):
    	...
    	while(openStack):
    		...
    		cur = openStack.remove(openStack[len(openStack)-1])
    		...
    		for value in range(position, len(self.state_lst), 1):
    			childrenStack.append(self.state_lst[value])
    			
    		#checks to see if the child is in the open or closed sets
    		while(childrenStack):
    			child = childrenStack.remove(openStack[len(childrenStack)-1])
    			#print(child)
    			#print("Here we are")
    			if (child not in openStack and child not in closedQueue):
    				#print("here is the step we are on")
    				openStack.append(child)
    	#print(childrenStack)
...
def main():
    ...
    p.depth_first_search(parent, goal)
